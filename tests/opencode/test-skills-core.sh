...
# Create skills in personal and plugin locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/plugin-skills/shared-skill"
mkdir -p "$TEST_HOME/plugin-skills/unique-skill"
...
cat > "$TEST_HOME/plugin-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
...
cat > "$TEST_HOME/plugin-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
...
const pluginDir = '$TEST_HOME/plugin-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', pluginDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: sp-ecc: prefix should force plugin
const forced = resolveSkillPath('sp-ecc:shared-skill', pluginDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to plugin
const unique = resolveSkillPath('unique-skill', pluginDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', pluginDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow plugin skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"sp-ecc"'; then
    echo "  [PASS] sp-ecc: prefix forces plugin resolution"
else
    echo "  [FAIL] sp-ecc: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"sp-ecc"'; then
    echo "  [PASS] Unique plugin skills are found"
else
    echo "  [FAIL] Unique plugin skills not found"
    exit 1
fi
