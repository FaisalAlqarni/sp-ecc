...
# Verifies that skills are resolved with correct priority: project > personal > plugin
...
# 1. Create in plugin location (lowest priority)
mkdir -p "$HOME/.config/opencode/sp-ecc/skills/priority-test"
cat > "$HOME/.config/opencode/sp-ecc/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Plugin version of priority test skill
---
# Priority Test Skill (Plugin Version)

This is the PLUGIN version of the priority test skill.

PRIORITY_MARKER_PLUGIN_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/sp-ecc/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Plugin version exists"
else
    echo "  [FAIL] Plugin version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides plugin
echo ""
echo "Test 2: Testing personal > plugin priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides plugin)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PLUGIN_VERSION"; then
    echo "  [FAIL] Plugin version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|plugin" | head -10
fi

# Test 3: Test that project overrides both personal and plugin
echo ""
echo "Test 3: Testing project > personal > plugin priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Plugin version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit sp-ecc: prefix bypasses priority
echo ""
echo "Test 4: Testing sp-ecc: prefix forces plugin version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load sp-ecc:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PLUGIN_VERSION"; then
    echo "  [PASS] sp-ecc: prefix correctly forces plugin version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] sp-ecc: prefix did not force plugin version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
